...
<link rel="stylesheet" type="text/css" href="/css/order2.css?v=20160713">
...
<script src="/js/order2.js?v=20160713" type="text/javascript"></script>
<script>
	function OrderSubmin(){
		var userInput = document.getElementsByClassName("userinf_input");
		var hidInput = document.getElementsByClassName("hid_input");
		for(var i=0;i<3;i++){
			hidInput[i].value = userInput[i].value;
		}
		$('#orderForm').submit();
	}
</script>
</head>	
<body id="mainframe" style="margin-bottom:50px;">
...
<form id="orderForm" class="hide" action="/order/create" method="post">
	...
	<input type="hidden" name="payment" value="<fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${totalPrice +5}"/>"/>
	<input class="hid_input" type="hidden" name="orderShipping.receiverName" value="${sessionScope.usercode.username}"/>
	<input class="hid_input" type="hidden" name="orderShipping.receiverMobile" value="${sessionScope.usercode.phone}"/>
	<input class="hid_input" type="hidden" name="orderShipping.receiverAddress" value="${sessionScope.usercode.address}"/>
  ...
  <div id="userAddrId" class="orderTbody ">
    ...
    <div class="orderItem">
        <div class="orderCurr" id="userAddress" style="display:block;">
          <ul>
            <li>用户名<input class="userinf_input" type="text" name="orderShipping.receiverName" style="margin-left:50px;border:1px solid black;" value="${sessionScope.usercode.username}"></li>
            <li>电话&nbsp;&nbsp;&nbsp;<input class="userinf_input" type="orderShipping.receiverMobile" name="phone" style="margin-left:50px;border:1px solid black;" value="${sessionScope.usercode.phone}"></li>
            <li>地址&nbsp;&nbsp;&nbsp;<input class="userinf_input" type="orderShipping.receiverAddress" name="address" style="margin-left:50px;border:1px solid black;width:500px;" value="${sessionScope.usercode.address}"></li>
          </ul>
        </div>
    </div>
  </div>
  ...
  <h3 class="orderTitle">支付及配送方式：
  	<span class="addrAlter hide" style="display: none;">[修改]</span>
  </h3>
  <div class="orderItem ">
		...
		<div class="shipment">
			<div class="shipOrder" id="ylsorder2">
				<div class="sOrders">
					...
					<div class="sOrders">
						<div class="scon">
							<strong>美团快运</strong>负责配送。<br>商品下单后尽快为您配送
						</div>
					</div>
  ...
  <div class="backToCart"><a style=" text-decoration:underline;" target="" href="/waimai_index">返回修改购物车</a></div>
				...
				<div class="orderPInfo">
					...
					<div class="pItem pPrice" style="position: relative; z-index: 99">
						¥<fmt:formatNumber
							...
							minFractionDigits="2" value="${cart.price }" />
					</div>
					...
					<div class="pItem pSubtotal">¥${cart.price * cart.num}</div> 
					<span class="clear"></span>
				</div>
				</c:forEach>
				<div class="orderPInfo order_xj">
					<span class="sp1">运费小计：</span> <span class="sp2">${5 }</span>
				</div>
			</div>
		</div>
	</div>
        ...
        <dl class="fl">
          <dd>
            <div>1 件商品，总商品金额：</div><span class="priceArea">¥<span id="yingProce">
				<fmt:formatNumber value="${totalPrice  }" maxFractionDigits="2" minFractionDigits="2" groupingUsed="true"/>
			</span></span>
          </dd>
          <dd>
            <div>运费金额：</div><span class="priceArea">¥<span id="shopping_price">${5 }</span></span>
          </dd>
          <dd>
            <div>应付总额：</div><span class="priceArea">¥<span id="allPrice">
            	<fmt:formatNumber value="${totalPrice +5 }" maxFractionDigits="2" minFractionDigits="2" groupingUsed="true"/>
            </span></span>
          </dd>
        </dl>
      </div>
  ...
  <div class="orderTFoot">
    ...
    <div class="submitOrder">
      <div class="orderSubmit">
      	<input id="save" name="save" onclick="OrderSubmin()" class="submitBtn" value="提交订单 " type="button"/>
      </div>
      <div class="orderAccount">
        <span class="t">应付总额：</span>
        <span class="p">¥</span>
        	<span id="countPrice" class="p"><fmt:formatNumber value="${totalPrice +5 }" maxFractionDigits="2" minFractionDigits="2" groupingUsed="true"/>
        </span>
      </div>
      <span class="clear"></span>
    </div>
